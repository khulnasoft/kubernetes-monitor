describe('extractNamespaceName()', function () {
    var apiToken = '46766a0a-ed0b-4e91-84c8-ea1c827f2a73';
    beforeEach(function () {
        jest.resetModules();
        process.env.KHULNASOFT_SYSDIG_ENDPOINT = 'https://api/v1/images/';
        process.env.KHULNASOFT_SYSDIG_TOKEN = '1432gtrhtrw32raf';
        process.env.KHULNASOFT_SERVICE_ACCOUNT_API_TOKEN = apiToken;
    });
    afterEach(function () {
        jest.restoreAllMocks();
        delete process.env.KHULNASOFT_CLUSTER_NAME;
        delete process.env.KHULNASOFT_SYSDIG_ENDPOINT;
    });
    test.each([
        [
            'cluster name with /',
    ...
    ])('%s', function (_testCaseName, _a) {
        var clusterNameEnvVar = _a.clusterNameEnvVar, wantClusterName = _a.wantClusterName, consoleLogCalledTimes = _a.consoleLogCalledTimes;
        if (clusterNameEnvVar) {
            process.env.KHULNASOFT_CLUSTER_NAME = clusterNameEnvVar;
        }
        var consoleSpy = jest.spyOn(console, 'log').mockReturnValue();
        var config = require('../../src/common/config').config;
        expect(config.CLUSTER_NAME).toBe(wantClusterName);
        expect(consoleSpy).toHaveBeenCalledTimes(consoleLogCalledTimes);
        delete process.env.KHULNASOFT_SYSDIG_TOKEN;
    });
    it('loads the expected configuration values', function () {
        var config = require('../../src/common/config').config;
    ...
    it('cannot load sysdig API and JWT values if it is not enabled', function () {
        delete process.env.KHULNASOFT_SYSDIG_ENDPOINT;
        delete process.env.KHULNASOFT_SYSDIG_TOKEN;
        var config = require('../../src/common/config').config;
