...
All integration tests determine the image to be tested based on the environment variable called `KUBERNETES_MONITOR_IMAGE_NAME_AND_TAG`, and fallback to `khulnasoft/kubernetes-monitor:local`, meaning one has to make sure the image desired to be tested is properly named and tagged.
...
Tilt allows you to run and debug the khulnasoft-monitor while it is running in a container. Tilt deploys the khulnasoft-monitor using the same Helm chart that we publish to users.

You can download Tilt from the [Tilt GitHub repository](https://github.com/tilt-dev/tilt#install-tilt).

### Start a debugging session ###

First, ensure you have the khulnasoft-monitor namespace set up and the khulnasoft-monitor Secret with your integration ID and dockercfg (as per the prerequisites for installing khulnasoft-monitor).
...
... it means that the `readOnlyRootFilesystem` protection on the khulnasoft-monitor Helm Deployment causes issues with Tilt. This can be fixed by removing the `readOnlyRootFilesystem: true` value from the Helm chart located in `khulnasoft-monitor/templates/deployment.yaml`.
