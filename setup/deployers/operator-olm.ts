...
const PULL_KUBERNETES_OPERATOR_EVENT =
  'Pulling image "docker.io/khulnasoft/kubernetes-operator:';
...
async function waitToDeployKubernetesOperator(
  ...
  for (let attempt = 0; attempt < 60; attempt++) {
    await kubectl.deleteDeployment('khulnasoft-operator', namespace);
...
async function deployKubernetesMonitor(
  ...
  const overriddenOperatorSource = 'khulnasoft-monitor-catalog-source.yaml';
  ...
  await kubectl.waitForDeployment('khulnasoft-operator', 'marketplace');
  await kubectl.waitForCRD('khulnasoftmonitors.charts.helm.k8s.io');

  const overriddenCustomResource = 'khulnasoft-monitor-custom-resource-k8s.yaml';
